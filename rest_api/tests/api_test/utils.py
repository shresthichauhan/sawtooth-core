...
def get_batches(head_id=None , id=None , start=None , limit=None , reverse=None, count=None):  
    if all(v is not None for v in [head_id , id]):
    ...
    if count is not None:
        response = query_rest_api('/batches?count=%s'% count)
        return response
    if reverse:
...
def create_invalid_intkey_transaction(verb, name, value, deps, signer):
    payload = IntKeyPayload(
        verb=verb, name=name, value=value)
